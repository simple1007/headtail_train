...
from autospace import AutoSpace
autospace = AutoSpace()
# print(root)
tok_max_len = 230
# print('ht',root)
...
import tokenizer_eomi as tokenizer
...
class PosTaggerModelPeftTorchBERT(nn.Module):
    def __init__(self,model,maxlen,hidden_state,tag_len):
        super(PosTaggerModelPeftTorchBERT,self).__init__()
        ...
        self.emb = nn.Embedding(len(bigram.keys()),80)#bilm.emb#nn.Embedding(len(bigram.keys()),80)
    ...
    def forward(self,bi,data,masks,segments):
        # 
        # comp = (bi == 1)
        # comp = torch.tensor(comp,dtype=torch.float32)
        # print(comp.shape)
        # exit()
        emb = self.emb(bi)
        # comp = emb + (comp * -1e-9)
        ...
        # emb_lstm = self.emb_lstm(emb)
        # emb_lstm2 = self.emb_lstm2(emb)
        # emb_lstm3 = self.emb_lstm3(emb)
        emb_lstm,_ = self.emb_lstm(emb)
        # emb_lstm2,_ = self.emb_lstm2(emb)
        # emb_lstm3,_ = self.emb_lstm3(emb)
        
        # emb_lstm = self.quant1(emb_lstm)
        # print(emb_lstm.shape)
        # emb_lstm = torch.sum(emb_lstm,dim=-2) / 200
        # print(emb_lstm.shape)
        
        #emb_lstm[:,-1,:]
        # print(emb_lstm)
        # emb_lstm = emb_lstm.view(bi.size(0),1,emb_lstm.shape[1])
        
        # print(emb_lstm.shape)
        # print(len(t),len(emb_lstm))
        # print(emb_lstm.shape)
        # emb_lstm = emb_lstm.view(bi.shape[0],emb_lstm.size(0),emb_lstm.size(1))
        # print(emb_lstm.shape)
        # bert_output
        # data = self.quant2(data)
        # masks = self.quant3(masks)
        # qk = torch.bmm(emb_lstm1,torch.transpose(emb_lstm2,2,1))
        
        # qk += (comp * -1e-9)
        
        # qk = self.softmax(qk)
        # emb_lstm = torch.bmm(qk,emb_lstm3)
        # print(emb_lstm.shape)
        bert_output = self.bert(input_ids=data,attention_mask = masks)[0]#,token_type_ids = segments)[0]
        # print(bert_output.shape)#.shape,bert_output[1].shape)
        bert_output = self.sub_layer(bert_output)
        # output = torch.concat([emb_lstm,bert_output[:,1:,:]],dim=-2)
        output = bert_output + emb_lstm#torch.concat([emb_lstm,bert_output[:,1:,:]],dim=-1)
        # output = torch.add([emb_lstm,bert_output],dim=-1)
        # output = emb_lstm + bert_output
        # output = bert_output
        # print(output.shape)
        output,_ = self.bilstm(output)#(output)
        # print(output.shape)
...
class TK_Model(nn.Module):
    def __init__(self):
        ...
        self.bilstm = nn.LSTM(128,256,bidirectional=True)
        self.bilstm_bi = nn.LSTM(128,256,bidirectional=True)

        self.output_tag = nn.Linear(256 * 2,5)
        self.softmax = nn.LogSoftmax(dim=-1)
...
def thread_tok():
    ...
    tok_model = TK_Model()
    tok_model.load_state_dict(torch.load(root+'/tokenizer_model/model_3'))
...
def thread_pos():
    ...
    with open("posvocab.pkl","rb") as f:
        posvocab = pickle.load(f)
    pos_model = PosTaggerModelPeftTorchBERT(model,None,None,tag_len)
    pos_model.load_state_dict(torch.load(root + os.sep + "bert/model"))
...
def get_tok(line,result,tok_model,device,verbose=0):
    ...
    tok = tokenizer.predict(tok_model,line,device,verbose=verbose)
    #print(tok)
    # token = tok[0].split(' ')
    # result = []
    result['1'] = tok
...
def get_cnoun(text):
    ...
    for txt_ in text:
        ...
        for txt in txt_.split():
            ...
            if ht[0] == "":
                continue
            if (ht[1]=="NNP" or ht[1]=="NNG") and len(ht[0]) > 3:# and iscnoun(ht[0]):
...
def analysis(x,verbose=0):
    ...
    get_pos(tok_,tok,result[0],pos_model,device,posvocab)
    # X = []
    pos = result[0]['pos']
    # print(pos)
    # pos = get_cnoun(pos)
        ...
        if mode == 'file':
            ...
            with tqdm(total=count) as pbar:
                ...
                if len(X) > 0:
                    ...
                    get_tok(X,result[0],models[0][0])
                    
                    tok_ = [[]]
                    for tk in result[0]['1']:
                        # print(tk)
                        tok_[0].append(tk.replace('+',' '))
                        
                    get_pos(tok_[0],result[0]['1'],result[0],models[1][0])
                    # pos = tok
                    # print(pos)
                    # pos_ = []
                    # for p, df in zip(pos,dot_flag):
