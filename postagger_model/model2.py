...
class BiEmbedding(nn.Module):
    def __init__(self,config: TransformerConfig):
        ...
        self.norm = torch.nn.LayerNorm(self.config.dmodel)
        "self.biemb = nn.Embedding(148756,self.config.dmodel)"
        # self.lstm = nn.LSTM(self.config.dmodel,self.config.dmodel//2,batch_first=True,bidirectional=True)
        # self.layernorm = nn.LayerNorm(self.config.dmodel)
        self.dropout = nn.Dropout(config.dropout)
        "self.layer = nn.Linear(self.config.dmodel,self.config.dmodel)"
        # if not subwordvocab:
        if os.environ.get("HT","") != "":
            with open(os.environ["HT"]+os.sep+"subwordvocab.pkl","rb") as f:
                self.subwordvocab = pickle.load(f)
    ...
    def forward(self,x):
        ...
        # emb = emb + self.setbigram(x)
        # pidx = torch.arange(0,emb.shape[1]).to(x.device)
        # emb = emb + self.pe(pidx).unsqueeze(0)
        # emb = self.norm(emb)# / 3#self.layernorm(emb)
        # emb = self.norm(emb)
        # emb = self.dropout(emb)
        # emb = self.layer(emb)
        # print(emb.shape)
        return emb
    def setUnfold(self,unfold):
        self.unfold = unfold
    def setbigram(self,data):
        data_ = self.unfold#data.unfold(1,3,1)
...
if True:
    class PosLSTMMini2(nn.Module):
        def __init__(self,posvocab):
            ...
            # self.model2 = SelfEncoder(posvocab,biemb=True)
            # self.config = self.model.config
            
            # self.config2 = copy.deepcopy(self.config)
            # self.config2.vocab_size = len(self.subwordvocab.bigram2index)
            # self.model2 = SelfEncoder(posvocab,vocab_size=len(self.subwordvocab.bigram2index))
            # self.model2.config = self.config2
            # print(self.config)
            # state_dict = remove_prefix_from_state_dict(torch.load("selfmodel_mini_19",weights_only=True))
            ...
            # self.biemb = nn.Embedding(len(self.subwordvocab.bigram2index),self.config.dmodel)
            
            # self.lstm_enc = nn.Linear(self.config.dmodel,self.config.dmodel)#,batch_first=True,bidirectional=True,num_layers=2)
            # self.lstm = nn.LSTM(self.config.dmodel,self.config.dmodel//2,batch_first=True,bidirectional=True,num_layers=1)#nn.LSTM(self.config.dmodel,self.config.dmodel,batch_first=True)
            
            # self.layer = nn.AvgPool1d(3, stride=2)
            # self.layer = nn.Linear(self.config.dmodel,self.config.dmodel//2)
            # self.dropout = nn.Dropout(self.config.dropout)
            # self.layer = nn.Linear(self.config.dmodel,self.)
            # self.lstm = nn.LSTM(self.config.dmodel+90,len(posvocab.pos2index),batch_first=True,bidirectional=False,num_layers=2)
            # self.lstm_output = nn.LSTM(self.config.dmodel,self.config.dmodel//2,batch_first=True)
            self.emb = nn.Embedding(10001,dmodel)
            self.output = nn.LSTM(dmodel,dmodel//2,batch_first=True,bidirectional=True)
            self.sublayer = nn.Linear(dmodel,dmodel//4)
            self.act = nn.GELU()
            self.vocab_output = nn.Linear(dmodel//4,len(posvocab.pos2index))
            # self.tanh = nn.Tanh()
        ...
        def forward(self,data,unfold):
            ...
            # print(data.shape)
            # exit()
            # unfold = unfold.view(-1)
            # print(unfold.max())
            # print(len(self.subwordvocab.bigram2index))
            # exit()
            '''
            print(data)
            print(unfold)
            ...
            '''
            # print(unfold.shape)
            # biemb= self.biemb(unfold)
            # lstm,_ = self.lstm(biemb)
            
            # self.model.model.emb.setUnfold(unfold)
            # enc2 = self.model2(unfold,is_softmax=True)
            # enc = self.lstm_enc(enc)
            # norm = self.norm(enc + lstm)
            # concat_t = torch.concat([enc,lstm],dim=2)
            # exit()
            # print(data.shape)
            # print(lstm.shape)
            # print(enc.shape)
            
            # output = self.layer(enc) 
            
            # print(output.shape)
            # exit()
            # enc = self.tanh(enc)
            # enc,_ = self.lstm(enc)
            # enc = torch.concat([enc,lstm],dim=-1)
            # enc = self.norm(enc + enc2)
            # enc = torch.concat([enc,enc2],dim=-1)
            # print(enc.shape)
            # exit()
            ...
            # print(output.shape)
            # exit()
            # output,_ = self.lstm_output(enc)
            # output = self.vocab_output(enc)
            # output = self.softmax(output)
            return output#.reshape(output.shape)
    ...
    class PosLSTMMini2(nn.Module):
        def __init__(self,posvocab):
            ...
            # self.model2 = SelfEncoder(posvocab,biemb=True)
            # self.config = self.model.config
            self.config = TransformerConfig(pad_idx=posvocab.pad,numheads=8,group_attn=False,dmodel=256, dff=512,nlayer=6,maxlen=600,vocab_size=10001,dropout=0.1,warmup_steps=100)
        
            self.model = Encoder(self.config,last_layer=False,embedding=BiEmbedding)
            # self.config2 = copy.deepcopy(self.config)
            # self.config2.vocab_size = len(self.subwordvocab.bigram2index)
            # self.model2 = SelfEncoder(posvocab,vocab_size=len(self.subwordvocab.bigram2index))
            # self.model2.config = self.config2
            ...
            """self.sublayer = nn.Linear(self.config.dmodel,self.config.dmodel//4,bias=False)
            self.act = nn.GELU()
            self.layernorm = nn.LayerNorm(self.config.dmodel//4)"""
            self.vocab_output = nn.Linear(self.config.dmodel,len(posvocab.pos2index),bias=False)
            # self.tanh = nn.Tanh()
            # self.softmax = nn.LogSoftmax(dim=-1)
            self.init_()
            self.tag_len = len(posvocab.pos2index)
        ...
        def init_(self):
            ...
            self.vocab_output.bias = torch.nn.Parameter(torch.zeros(len(self.subwordvocab.pos2index)))
...
if True:
    class PosLSTMMini2CPP(nn.Module):
        def __init__(self,pad,outsize):
            ...
            self.sublayer = nn.Linear(dmodel,dmodel//4)
            self.act = nn.GELU()
            self.vocab_output = nn.Linear(dmodel//4,outsize)

            self.tag_len = outsize
...
class SelfEncoder(nn.Module):
    def __init__(self,pad,vocab_size=None,biemb=False):
        ...
        self.config = TransformerConfig(pad_idx=pad,numheads=8,group_attn=False,dmodel=128, dff=512,nlayer=6,maxlen=600,vocab_size=10001,dropout=0.02,warmup_steps=100)
        
        if vocab_size:
            self.config.vocab_size = vocab_size
            # self.config = TransformerConfig(pad_idx=posvocab.pad,numheads=8,group_attn=False,dmodel=256, dff=1024,nlayer=2,maxlen=600,vocab_size=vocab_size,dropout=0.1,warmup_steps=100)
        
        # self.config = TransformerConfig(pad_idx=posvocab.pad,numheads=8,group_attn=False,dmodel=256, dff=1024,nlayer=5,maxlen=600,vocab_size=10001,dropout=0.1,warmup_steps=100)
        ...
        # self.subwordvocab = posvocab
        # self.norm = torch.nn.LayerNorm(self.config.dmodel)
        # self.config = TransformerConfig(pad_idx=posvocab.pad,numheads=4,ngroup=2,group_attn=True,dmodel=256, dff=1024,nlayer=6,maxlen=600,vocab_size=15001,dropout=0.1,warmup_steps=100)
