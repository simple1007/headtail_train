...
def predictbi_pt(model,x,tk_line,device,lite=False,verbose=0):
    with torch.no_grad():
        ...
        bi_temp = torch.tensor(bi_temp).to(dtype=torch.long,device=device)
        x = torch.tensor(x).to(dtype=torch.long,device=device)
        uni = torch.tensor(uni).to(dtype=torch.long,device=device)
        masks = torch.tensor(masks).to(dtype=torch.long,device=device)
        segments = torch.tensor(segments).to(dtype=torch.long,device=device) 
        
        with torch.autocast(device_type="cuda",dtype=torch.bfloat16):
            result = model(uni,x,masks,segments)

        # print(result[0].shape,result[1].shape)
        
        _,result_ = torch.topk(result,k=1,dim=-1)#tf.argmax(result,axis=2).numpy()
        result_ = result_.view(result_.shape[0],result_.shape[1]).cpu().numpy()
        result_ = result_[:,1:]
        
        # _,result2_ = torch.topk(result[1],k=1,dim=-1)
        # result2_ = result2_.view(result2_.shape[0],result[1].shape[1]).cpu().numpy()
        # result2_ = result2_[:,1:]
        ...
        for l in result_:
            ...
            # l2 = l2.tolist()
            # print(l,l2)
            tagging.append(' '.join(posvocab.to_tag(l)))
        # print(tagging
        pos_res=tag_merge(tagging,tk_line,posvocab)
    return pos_res