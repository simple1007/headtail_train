...
# for line,(morph_,tag_) in enumerate(zip(morph_f,tag_f)):
    ...
    for i in morph_.split(' '):
        count_word[i] += 1
    tokens = tokenizer.tokenize('[CLS] '+ morph_+' [SEP]')
    tag__ = tag_.split()
    ...
    tag = None
    # print(tokens[1:-1],tag__)
    for tk in tokens[1:-1]:
        
        # print(tk,tag)
        if not tk.startswith("#"):
            tag = tag__.popleft()
            tags.append(("B_"+tag).replace("__","_"))
        else:
            tags.append(("I_"+tag).replace("__","_"))
    
    bis = []
    for i in range(len(tokens)-1):
        bi = ' '.join(tokens[i:i+2])        
        if bi not in token_bi_dict:
            token_bi_dict[bi] = tk_index
            tk_index += 1
        bis.append(token_bi_dict[bi])
    bis = bis[:max_len-2]
    bis = bis + [token_bi_dict["[PAD]"]] * (max_len - len(bis))
    x = []
    y = []
    tags = tags[:max_len-2]
    tags = ["[CLS]"]+tags+["[SEP]"]
    ...
    # y = y[:max_len-2] 
    y = y + [tag_dict["[PAD]"]] * (max_len - len(y))
    
    x = tokenizer(morph_,max_length=max_len,truncation=True,padding="max_length")["input_ids"]
    # print(x,morph_)
    # print(len(x),len(y),len(bis))
    X.append(x)
    Y.append(y)
    TK_BI.append(bis)
    # exit()
