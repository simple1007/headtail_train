...
def thread_tok():
    ...
    tok_model = TK_Model()
    tok_model.load_state_dict(torch.load(root+'/tokenizer_model_modu2/model'),dtype=torch.bfloat16)
...
def thread_pos():
    ...
    pos_model.load_state_dict(torch.load(root + os.sep + "torch_base_modu_kiwi/model"),dtype=torch.bfloat16)
    ...
    for x in X:
        ...
        for xx in x:
            ...
            if "+" in xx:
                ...
                # if not ht.startswith("VA") and ht.startswith("V") and hm[-1] in n_tags and len(hm) >= 3:
                ...
                # elif not ht.startswith("VA") and hm[-2:] in n_tags and len(hm) >= 3:
                #     ht = "V"
                ...
                #     # print(morphs)
                #     # exit()
                if not ht.startswith("VA") and ht.startswith("N") and hm[-1] == "들" and len(hm) >= 3:
                    hm = hm[:-1] +"/NNP+" + hm[-1]+"/"+"XSN" + "+" + t
                ...
                if len(m) == 0:
                    continue
                # if not t.startswith("VA") and t.startswith("V") and m[-1] in n_tags and len(m) >= 3:
                # # if m[-1] in n_tags and len(m) >= 3:
                #     t = "V"
                #     m = m[:-1] + "/NNP+" + m[-1] +"/" + t
                #     morphs = m
                # # elif t.startswith("V") and m[-2:] in n_tags and len(m) >=3:
                # elif not t.startswith("VA") and m[-2:] in n_tags and len(m) >=3:
                #     t = "V"
                #     m = m[:-2] + "/NNP+" + m[-2:] +"/" + t
                #     morphs = m
                if not t.startswith("VA") and t.startswith("N") and m[-1] == "들" and len(m) >= 3:
                    m = m[:-1] +"/NNP+" + m[-1]+"/"+"XSN" #+ "+" + t
                    morphs = m
                # print(morphs)
            # print(morphs)
