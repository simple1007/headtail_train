...
def make_batch(length,lstm_X,lstm_Y,BI):
    ...
    for lx,ly in zip(lstm_X,lstm_Y):
        ...
        lstm_y = ly[:mx_length] + [0] * (mx_length - len(ly))
...
def make_data(x,y,length,lstm_X,lstm_Y,BI):
    bi_npy = []
    # for xx in x.split():
    #     eomiaho.s
    ...
    # print(lstm_y)
    if False:
        lstm_y = y[:max_len] + [0] * (max_len -len(y))
...
if __name__ == "__main__":
    if not args.infer:
        ...
        optimizer = torch.optim.Adam(model.parameters(),lr=args.lr)#2e-4)#5e-5)#8e-5)#5e-5)
        if args.sepoch > 0:
        ...
        for e in range(args.sepoch,args.sepoch+EPOCH):
            ...
            for index in range(count_data):
                ...
                loss__ = train(config,optimizer,loss,model,xbi[0],xbi[1],y,device)
                running_loss += loss__
                end = datetime.now()
                cur = end - start
        ...
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu") # PyTorch v0.4.0
        print(device)
        sys.path.append(os.path.join(os.environ["Desktop"],"kjm","headtail"))
        os.environ["HT"] = os.path.join(os.environ["Desktop"],"kjm","headtail","postagger_model")
        score = 0
        testlen = 0
        model = TK_Model2(max_len,lstm_vocab)
        ...
        while False:
            text = input("sent: ")
            print(tokenizer.predict3ð(model,[text.strip()],device))
        ...
        with open("httk_x.txt",encoding="utf-8") as fx: 
            with open("httk_y.txt",encoding="utf-8") as fy:
                ...
                for x,y in zip(fx,fy):
                    counttmp += 1
                    if counttmp <= (count_data + validation_data) * args.BATCH:
                        continue
