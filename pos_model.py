...
class PosLSTMMini2(nn.Module):
    def __init__(self,posvocab):
        ...
        # self.output = nn.LSTM(self.config.dmodel,len(posvocab.pos2index),batch_first=True)
        self.vocab_output = nn.Linear(self.config.dmodel*2,len(posvocab.pos2index))
        # self.tanh = nn.Tanh()
        # self.softmax = nn.LogSoftmax(dim=-1)
        self.tag_len = len(posvocab.pos2index)
...
class SelfEncoder(nn.Module):
    def __init__(self,posvocab,vocab_size=None,biemb=False):
        super(SelfEncoder,self).__init__()
        self.config = TransformerConfig(pad_idx=posvocab.pad,numheads=8,group_attn=False,dmodel=128, dff=800,nlayer=2,maxlen=600,vocab_size=10001,dropout=0.1,warmup_steps=100)
